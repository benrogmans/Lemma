...
use lemma::{Engine, LoadSource, OperationResult, ResourceLimits};
...
fn lemma_load<'a>(
    ...
    let source = if source_label.trim().is_empty() {
        ...
        LoadSource::Labeled(source_label.as_str())
    };
...
fn lemma_run<'a>(
    ...
    match engine.run(&spec, Some(&effective), values) {
        Ok(response) => {
...
fn lemma_format<'a>(env: Env<'a>, code: String) -> NifResult<Term<'a>> {
    match lemma::format_source(&code, LoadSource::INLINE_KEY) {
