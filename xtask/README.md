...
|---------|-------|---------|
| `cargo run -p xtask -- [precommit] [--fuzz]` | `cargo precommit [--fuzz]` | `versions-verify`, mix precommit (hex), vscode `npm ci` + compile + vsce package + publish CLI smoke (`@node-rs/crc32`, `ovsx`, `vsce`; npm/Maven warnings are errors), `fmt --check`, clippy host + wasm32 (`-D warnings`), nextest (`--all-features`), npm WASM build+test, Maven `./mvnw -B verify` (after `lemma_jni` build; `-Werror` + `failOnWarnings` + `doclint=all` + `[WARNING]` gate), cargo-deny, `coverage all --check`. With `--fuzz` (CI): 30 minutes total across `engine/fuzz` targets (nightly + `cargo-fuzz`). Bare `cargo precommit` skips fuzz. |
