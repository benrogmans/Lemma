...
Lemma is designed to run wherever your rules need to live. Install the CLI to evaluate specs from the terminal, embed the engine directly in Rust, JavaScript/TypeScript, or Elixir, serve specs over HTTP or MCP, or pull a Docker image for containerized workloads. For language bindings and the full SDK documentation, see [Tools & SDKs](tools/readme.md).
...
Lemma can be embedded directly in Rust, JavaScript/TypeScript, and Elixir. See [Tools & SDKs](tools/readme.md) for the full SDK documentation for each language. More languages are coming soon.
