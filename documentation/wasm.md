...
- **`Engine`** — `load`, `load_batch`, `fetch`, `list`, `format_repository`, `schema`, `run`, `format`. `list()` always includes embedded repo `lemma` (`spec units`); use `format_repository("lemma")` for canonical stdlib source.
