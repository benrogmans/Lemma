            ...
            for (world, accumulated_constraint) in current_worlds {
                ...
                for (branch_idx, branch) in rule_node.branches.iter().enumerate() {
                    ...
                    let substituted_condition =
                        substitute_rules_in_expression(&branch.condition, &new_world, self.plan)?;
            ...
            if let Some(&branch_idx) = world.get(target_rule_path) {
                if let Some(rule_node) = self.rule_cache.get(target_rule_path) {
                    if branch_idx < rule_node.branches.len() {
                        ...
                        let substituted_result =
                            substitute_rules_in_expression(&branch.result, &world, self.plan)?;
...
fn substitute_rules_recursive(
    ...
    match &expr.kind {
            ...
            if let Some(&branch_idx) = world.get(rule_path) {
                if let Some(rule) = plan.get_rule_by_path(rule_path) {
                    if branch_idx < rule.branches.len() {
                        ...
                        let result =
                            substitute_rules_recursive(&branch.result, world, plan, visited_rules);
        ...
        ExpressionKind::RuleReference(_) => Err(LemmaError::Engine(
...
fn hydrate_facts_recursive(
    ...
    match &expr.kind {
        ...
        ExpressionKind::FactReference(_) => Err(LemmaError::Engine(
...
mod tests {
    ...
    fn test_constant_fold_arithmetic() {
        ...
        let expr = Expression::new(
            ExpressionKind::Arithmetic(Box::new(left), ArithmeticComputation::Add, Box::new(right)),
            test_source(),
        );
