...
impl Constraint {
    ...
    pub fn from_expression(expr: &crate::Expression) -> LemmaResult<Constraint> {
        ...
        match &expr.kind {
            ...
            ExpressionKind::FactPath(fact_path) => {
                Ok(Constraint::Fact(fact_path.clone()))
            }

            ExpressionKind::Comparison(left, op, right) => {
                Self::from_comparison(left, op, right)
            }
        ...
        if op.is_equal() {
            if let ExpressionKind::Comparison(inner_left, inner_op, inner_right) = &left.kind {
                if let ExpressionKind::Literal(LiteralValue::Boolean(bool_val)) = &right.kind {
                    let inner_constraint = Self::from_comparison(inner_left, inner_op, inner_right)?;
    ...
    pub fn collect_facts(&self) -> Vec<FactPath> {
        ...
        facts.sort_by(|a, b| a.to_string().cmp(&b.to_string()));
        facts.dedup();
        facts
    }
...
fn constraints_structurally_equal(a: &Constraint, b: &Constraint) -> bool {
    match (a, b) {
        ...
        (
            Constraint::Comparison { fact: f1, op: o1, value: v1 },
            Constraint::Comparison { fact: f2, op: o2, value: v2 },
        ) => f1 == f2 && o1 == o2 && v1 == v2,
...
fn from_bool_expr(bool_expr: &boolean_expression::Expr<usize>, atoms: &[Constraint]) -> Constraint {
    ...
    match bool_expr {
        ...
        Expr::Terminal(i) => {
            atoms.get(*i).cloned().unwrap_or(Constraint::False)
        }
        Expr::Not(inner) => {
...
fn format_with_parens(inner: &Constraint, parent: &Constraint) -> String {
    let needs_parens = matches!((parent, inner), (Constraint::And(_, _), Constraint::Or(_, _)));
...
mod tests {
    ...
    fn test_simplify_contradiction() {
        ...
        let c1 = comparison("x", ComparisonComputation::Equal(EqualityNotation::Symbol), 1);
        let c2 = comparison("x", ComparisonComputation::Equal(EqualityNotation::Symbol), 2);
