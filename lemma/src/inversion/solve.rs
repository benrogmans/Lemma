    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            SolveError::RuleReferenceFound => {
                write!(f, "Rule reference found - should be substituted before solving")
            }
        }
    }
}
...
fn find_unknown_facts(
    ...
    unknown_facts.sort_by(|a, b| a.to_string().cmp(&b.to_string()));
    unknown_facts.dedup();
    unknown_facts
}
...
fn has_supported_operations(expression: &Expression) -> bool {
    match &expression.kind {
        ...
        ExpressionKind::Arithmetic(left, operation, right) => {
            ...
            supported_operation
                && has_supported_operations(left)
                && has_supported_operations(right)
        }
...
pub fn solve_arithmetic_batch(
    ...
    provided_facts: &HashSet<FactPath>,
) -> Vec<(super::world::WorldArithmeticSolution, LiteralValue, std::collections::HashMap<FactPath, super::Domain>)> {
