...
#[test]
fn test_parentheses_syntax_evaluation() {
    use lemma::Engine;

    // Test that parentheses syntax evaluates correctly, not just parses
    let code = r#"
doc test
fact x = true
fact y = false
...
"#;
    ...
    assert!(
        result.is_ok(),
        "Failed to parse document with parentheses syntax: {:?}",
        result.err()
    );

    // Evaluate and verify results
    let eval_result = engine.evaluate("test", vec![], std::collections::HashMap::new());
    assert!(
        eval_result.is_ok(),
        "Failed to evaluate: {:?}",
        eval_result.err()
    ...
    // Verify not(x) evaluates to false (since x = true)
    ...
    // Verify combined expression works
    // not(x) where x=true gives false, and (sqrt(16) == 4) gives true
