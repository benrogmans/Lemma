    ...
    match &proof.tree {
        lemma::proof::ProofNode::Branches { matched, .. } => {
            ...
            match &*matched.result {
                ...
                other => panic!(
                    "Expected Computation node in matched branch result, got {:?}",
                    other
                ),
            }
        }
    ...
    match &proof.tree {
        lemma::proof::ProofNode::Branches { matched, .. } => {
            ...
            match &*matched.result {
                        ...
                        lemma::proof::ProofNode::RuleReference {
                            ...
                            match &**expansion {
                                lemma::proof::ProofNode::Branches {
                                    ...
                                    other => panic!(
                                        "Expected Computation in doubled expansion, got {:?}",
                                        other
                                    ),
                                },
                ...
                other => panic!(
                    "Expected Computation in matched branch result, got {:?}",
                    other
                ),
            }
        }
...
"#;
    ...
    let computation = matched["result"]["Computation"]
        .as_object()
        .unwrap_or_else(|| {
            panic!(
