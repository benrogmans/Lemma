...
//! - Proof generation
//!
//! Note: Algebraic solving (e.g., solving `price * 5 = 50` for price) is NOT yet
//! implemented in the world-based inversion module. Tests for that capability
//! are in the legacy inversion_smoke.rs file.
...
fn boolean_multiple_unless_clauses_specific_target() {
    ...
    assert_eq!(response.solutions.len(), 1, "should have exactly 1 solution for 10%");
...
fn boolean_multiple_unless_clauses_any_value() {
    ...
    assert!(outcomes.iter().any(|o| o.contains("0")), "should have 0% outcome");
    assert!(outcomes.iter().any(|o| o.contains("10")), "should have 10% outcome");
    assert!(outcomes.iter().any(|o| o.contains("20")), "should have 20% outcome");
    assert!(outcomes.iter().any(|o| o.contains("5")), "should have 5% outcome");
}
...
fn text_enumeration_with_veto() {
    ...
    assert_eq!(response.solutions.len(), 1, "should have exactly 1 solution");
    
    let status_domain = response.domains.get(0)
        .and_then(|d| d.get(&FactPath::local("status".to_string())));
    
    assert_eq!(
        status_domain,
        Some(&Domain::Enumeration(vec![LiteralValue::Text("approved".to_string())])),
...
fn any_value_includes_veto_outcomes() {
    ...
    assert_eq!(response.solutions.len(), 4, "should have 4 solutions (false, true, 2 vetos)");
    
    let has_veto = response.solutions.iter().any(|s| {
        s.outcome.to_string().contains("veto")
    });
    
    assert!(has_veto, "should include veto outcomes when using any_value");
}
...
fn veto_boundary_produces_range() {
    ...
    assert_eq!(response.solutions.len(), 2, "should have 2 solutions (true, veto)");
    
    let true_solution = response.solutions.iter()
    ...
    let true_idx = response.solutions.iter()
        .position(|s| s.outcome == OperationResult::Value(LiteralValue::Boolean(BooleanValue::True)))
        .unwrap();
    
    let weight_domain = response.domains.get(true_idx)
        .and_then(|d| d.get(&FactPath::local("weight".to_string())));
    
    assert!(weight_domain.is_some(), "should have weight domain for true solution");
    ...
    match weight_domain {
        ...
        _ => panic!("weight should be Range or Complement, got {:?}", weight_domain)
    }
}
...
fn rule_references_expand_correctly() {
    ...
    assert_eq!(response.solutions.len(), 1, "should have exactly 1 solution for 15%");
    
    let points_domain = response.domains.get(0)
    ...
    match points_domain.unwrap() {
        Domain::Range { min, max } => {
            ...
            match max {
                Bound::Exclusive(v) => {
                    assert_eq!(*v, LiteralValue::number(1000), "max should be 1000 exclusive");
                }
...
fn complex_pricing_with_member_coupon_combo() {
    ...
    assert_eq!(response.solutions.len(), 1, "should have exactly 1 solution for 20%");
...
fn multi_branch_trade_in_values() {
    ...
    let response = engine
        .invert("order", "trade_in_value", Target::any_value(), HashMap::new())
        .expect("invert should succeed");
    ...
    assert!(outcomes.contains(&"50".to_string()), "should have outcome 50");
    assert!(outcomes.contains(&"30".to_string()), "should have outcome 30");
    assert!(outcomes.contains(&"10".to_string()), "should have outcome 10");
}
...
fn multi_branch_specific_target_filtering() {
    ...
    for (target_value, expected_condition_value) in [
        (50, "excellent"),
        (30, "good"),
        (10, "fair"),
    ] {
        let response = engine
            ...
            .expect(&format!("invert should succeed for value {}", target_value));
...
fn algebraic_solve_simple_multiplication() {
    ...
    let price_domain = response.domains.get(0)
...
fn algebraic_solve_simple_addition() {
    ...
    let x_domain = response.domains.get(0)
...
fn algebraic_solve_chained_operations() {
    ...
    let hours_domain = response.domains.get(0)
...
fn algebraic_solve_division() {
    ...
    let total_domain = response.domains.get(0)
    ...
    match total_domain.unwrap() {
        Domain::Enumeration(values) => {
            assert_eq!(values.len(), 1, "should have exactly 1 value");
            assert_eq!(values[0], LiteralValue::number(30), "total_servings should be 30");
        }
        _ => panic!("total_servings domain should be Enumeration, got {:?}", total_domain)
    }
}
...
fn algebraic_solve_subtraction_from_left() {
    ...
    let original_domain = response.domains.get(0)
        .and_then(|d| d.get(&FactPath::local("original_price".to_string())));

    assert!(original_domain.is_some(), "should have original_price domain");

    match original_domain.unwrap() {
        Domain::Enumeration(values) => {
            assert_eq!(values.len(), 1, "should have exactly 1 value");
            assert_eq!(values[0], LiteralValue::number(100), "original_price should be 100");
        }
        _ => panic!("original_price domain should be Enumeration, got {:?}", original_domain)
    }
}
...
fn algebraic_solve_subtraction_from_right() {
    ...
    let payment_domain = response.domains.get(0)
    ...
    match payment_domain.unwrap() {
        ...
        _ => panic!("payment domain should be Enumeration, got {:?}", payment_domain)
    }
}
