...
use lemma::documentation::{GuideTopic, EVALUATE_GUIDE};
use lemma::Engine;
use serde_json::Value;

use crate::error::{map_engine_error, ToolError};
...
pub fn evaluate(engine: &Engine, args: &Value) -> Result<String, ToolError> {
    ...
    let spec_name = lemma::parse_spec_set_id(spec_set_id).map_err(map_engine_error)?;
...
pub fn check(args: &Value) -> Result<String, ToolError> {
    ...
    let mut sources: Vec<(lemma::SourceType, String)> = Vec::with_capacity(sources_arr.len());
    for (i, entry) in sources_arr.iter().enumerate() {
        ...
        let source_type =
            lemma::SourceType::from_binding_label(label).map_err(ToolError::invalid_arguments)?;
...
fn resolve_effective(args: &Value) -> Result<lemma::DateTimeValue, ToolError> {
    match args.get("effective") {
        None | Some(Value::Null) => lemma::resolve_effective(None).map_err(map_engine_error),
        Some(Value::String(raw)) => lemma::resolve_effective(Some(raw)).map_err(map_engine_error),
